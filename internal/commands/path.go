...
//
// Direction choice: we ALWAYS search a → b (a depends on b
// transitively). To find paths in the reverse direction
// ("everything that depends on #1"), use `tsk graph --reachable`
// (shipping in the same tick). Bidirectional was considered and
// dropped — the right tool depends on the question, and asking
// the user to be explicit is better than guessing.
...
Use ` + "`tsk graph --reachable <id>`" + ` for the reverse question
("everything that depends on #X"); ` + "`tsk path`" + ` is one-to-one only.
...
`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return emitPath(cmd.OutOrStdout(), s, from, to, path, asJSON)
		},
	}
...
func emitPath(w io.Writer, s *store.Store, from, to int, path []int, asJSON bool) error {
	if asJSON {
		doc := pathJSON{
			From:  from,
			To:    to,
	...
	if len(path) == 0 {
		pf(w, "no dependency path from #%d to #%d\n", from, to)
		return silentExit{code: 1}
	}
